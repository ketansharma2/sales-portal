...
export default function JobRequirementsPage() {
  ...
  useEffect(() => {
    ...
    const fetchBranches = async () => {
      ...
      try {
        ...
        const response = await API.apiGet(`/api/corporate/crm/branches?client_id=${selectedClient.client_id}`);
        const data = await response.json();
        if (isMounted && data.success) {
  ...
  useEffect(() => {
    ...
    const fetchAssignments = async () => {
      try {
        const response = await API.apiGet("/api/corporate/crm/jobpost/make");
        const data = await response.json();
        if (isMounted && data.success) {
  ...
  const handleAssign = async () => {
    ...
    if (!selectedRequirement) {
      ...
      try {
        const response = await API.apiPost("/api/corporate/crm/jobpost/make", {
    ...
    assigned_to: inlineForm.assigned_to
        ...
        if (data.success) {
          ...
          // Refresh assignments
          const refreshResponse = await API.apiGet("/api/corporate/crm/jobpost/make");
