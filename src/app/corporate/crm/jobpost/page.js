...
export default function JobRequirementsPage() {
  ...
  useEffect(() => {
    ...
    const fetchBranches = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/corporate/crm/branches?client_id=${selectedClient.client_id}`, {
          headers: {
            'Authorization': `Bearer ${session.access_token}`
          }
        });
  ...
  useEffect(() => {
    ...
    const fetchAssignments = async () => {
      try {
        const session = JSON.parse(localStorage.getItem('session') || '{}');
        const response = await fetch('/api/corporate/crm/jobpost/make', {
          headers: {
            'Authorization': `Bearer ${session.access_token}`
          }
        });
    ...
    if (!selectedRequirement) {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Refresh assignments
          const refreshResponse = await fetch('/api/corporate/crm/jobpost/make', {
            headers: {
              'Authorization': `Bearer ${session.access_token}`
            }
          });
