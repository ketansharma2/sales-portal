...
export default function TLCorporateTargetPage() {
  ...
  console.log('Corporate TL Target Page component loaded');
  ...
  const fetchTeamUsers = async () => {
    try {
      const response = await API.apiGet('/api/corporate/tl/team-users');
  ...
  const fetchMyTargets = async () => {
    try {
      const response = await API.apiGet('/api/corporate/tl/my-targets');
    ...
    try {
      ...
      for (const combo of uniqueCombos) {
        ...
        if (trackerSentTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/my-tracker-sent-achievement?month=${combo.month}&year=${combo.year}`);
      ...
      for (const combo of uniqueCombos) {
        ...
        if (accuracyTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/my-accuracy-achievement?month=${combo.month}&year=${combo.year}`);
      ...
      for (const combo of uniqueCombos) {
        ...
        if (joiningTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/my-joining-achievement?month=${combo.month}&year=${combo.year}`);
      ...
      for (const combo of uniqueCombos) {
        ...
        if (cvParseTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/my-cv-parse-achievement?month=${combo.month}&year=${combo.year}`);
      ...
      for (const combo of uniqueCombos) {
        ...
        if (conversionTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/my-conversion-achievement?month=${combo.month}&year=${combo.year}`);
  ...
  const fetchTeamTargets = async () => {
    console.log('fetchTeamTargets called');
    try {
      ...
      const response = await API.apiGet(`/api/corporate/tl/team-targets?${params.toString()}`);

      const result = await response.json();
      console.log('fetchTeamTargets response:', result);

      if (result.success && result.data) {
        console.log('team targets data length:', result.data.length);
        setTeamTargets(result.data);
        // Fetch dynamic achievements for team targets
  ...
  const fetchDynamicAchievements = async (targets) => {
    try {
      ...
      // Get unique combinations for CV Parse, Tracker Sent, Accuracy, Conversion, and Joining KPIs
      const uniqueCombos = [...new Set(targets.map(t => `${t.assignedToId}|${t.month}|${t.year.toString()}`))].map(combo => {
        const [assignedToId, month, year] = combo.split('|');
        return { assignedToId, month, year };
      });
      ...
      for (const combo of uniqueCombos) {
        ...
        if (cvParseTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/cv-parse-achievement?month=${combo.month}&year=${combo.year}&assigned_to_id=${combo.assignedToId}`);

            const result = await response.json();

            if (result.success && result.data) {
              const key = `${combo.assignedToId}|${combo.month}|${combo.year}`;
              if (!achievements[key]) achievements[key] = {};
              achievements[key].cvParse = {
                achieved: result.data.achieved,
                percentage: result.data.percentage
            ...
            console.error(`Error fetching Corporate TL CV parse achievement for ${combo.month} ${combo.year} ${combo.assignedToId}:`, error);
          }
        }
      }
      ...
      for (const combo of uniqueCombos) {
        ...
        if (trackerSentTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/tracker-sent-achievement?month=${combo.month}&year=${combo.year}&assigned_to_id=${combo.assignedToId}`);
            ...
            console.error(`Error fetching Corporate TL tracker sent achievement for ${combo.month} ${combo.year} ${combo.assignedToId}:`, error);
          }
        }
      }
      ...
      for (const combo of uniqueCombos) {
        ...
        if (accuracyTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/accuracy-achievement?month=${combo.month}&year=${combo.year}&assigned_to_id=${combo.assignedToId}`);
            ...
            console.error(`Error fetching Corporate TL accuracy achievement for ${combo.month} ${combo.year} ${combo.assignedToId}:`, error);
          }
        }
      }
      ...
      for (const combo of uniqueCombos) {
        ...
        if (conversionTargets.length > 0) {
          try {
            const response = await API.apiGet(`/api/corporate/tl/conversion-achievement?month=${combo.month}&year=${combo.year}&assigned_to_id=${combo.assignedToId}`);
            ...
            console.error(`Error fetching Corporate TL conversion achievement for ${combo.month} ${combo.year} ${combo.assignedToId}:`, error);
          }
        }
      }
      ...
      for (const combo of uniqueCombos) {
        ...
        if (joiningTargets.length > 0) {
          try {
           const response = await API.apiGet(`/api/corporate/tl/joining-achievement?month=${combo.month}&year=${combo.year}&assigned_to_id=${combo.assignedToId}`);
            ...
            console.error(`Error fetching Corporate TL joining achievement for ${combo.month} ${combo.year} ${combo.assignedToId}:`, error);
          }
        }
      }
      ...
      console.error('Error fetching Corporate TL dynamic achievements:', error);
    }
  };
  ...
  const handleOpenModal = (item = null) => {
    if (item && item.id) {
        ...
        setForm({
            ...
            department: "Delivery", sector: "Corporate", role: "RC", assignedTo: item.assignedTo || "",
            targetList: [{ 
        ...
        setForm({
            ...
            department: "Delivery", sector: "Corporate", role: "RC", assignedTo: "",
    ...
    if (editId) {
        ...
        const updateInAPI = async () => {
            try {
                ...
                const response = await API.apiPut('/api/corporate/tl/team-targets', {
    target_id: editId,
    year: form.year,
    month: form.month,
    ...
    total_target: t.target
        ...
        const saveToAPI = async () => {
            try {
                ...
                const userResponse = await API.apiGet('/api/corporate/tl/team-users');
                ...
                const response = await API.apiPost('/api/corporate/tl/team-targets', {
    year: form.year,
    month: form.month,
    working_days: form.workingDays,
    role: form.role,
  ...
  console.log('filteredTeamTargets length:', filteredTeamTargets.length);
                  ...
                  {loading ? (
                     <tr><td colSpan="10" className="p-12 text-center text-gray-400 font-bold uppercase tracking-widest">Loading Targets...</td></tr>
                      ...
                      filteredTeamTargets.map((item) => {
                          ...
                          const isConversionKPI = item.kpi_metric?.toLowerCase() === 'conversion';
                          const isJoiningKPI = item.kpi_metric?.toLowerCase() === 'joining';
                          const dynamicKey = `${item.assignedToId}|${item.month}|${item.year.toString()}`;
                          ...
                          const displayTarget = item.frequency === 'Daily' ? (item.target * item.workingDays) : item.target;
                          const achievedValue = dynamicData ? dynamicData.achieved : (item.achieved || 0);
                          const percentage = displayTarget > 0 ? Math.round((achievedValue / displayTarget) * 100) : 0;

                        console.log('item:', item.kpi_metric, item.assignedToId, item.month, item.year, 'achievedValue:', achievedValue, 'percentage:', percentage, 'displayTarget:', displayTarget);
                          let percColor = "text-red-600 bg-red-50 border-red-200";
                          if(percentage >= 100) percColor = "text-emerald-700 bg-emerald-50 border-emerald-200";
                          else if(percentage >= 50) percColor = "text-amber-600 bg-amber-50 border-amber-200";
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-5 max-h-[80vh] overflow-y-auto custom-scrollbar flex flex-col">
                    ...
                    <div className="pt-4 border-t border-gray-100 mt-auto">
                        <button onClick={handleSaveTarget} disabled={savingTarget} className="w-full bg-indigo-700 hover:bg-indigo-800 py-3 rounded-xl font-black uppercase tracking-widest text-white shadow-md flex items-center justify-center gap-2 transition-colors text-xs disabled:opacity-60 disabled:cursor-not-allowed">
