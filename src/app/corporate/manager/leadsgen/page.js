...
const fetchCrmDetails = async (clientId) => {
  try {
    ...
    if (data.success) {
      return data; // Return the full response with data property
    } else {
  ...
  const handleAction1 = async (id, action) => {
  ...
  if (action === 'crmView') {
    ...
    if (crmDetails && crmDetails.data) {
      // Access the data property from the response
      const apiData = crmDetails.data;
    ...
    } else if (crmDetails && !crmDetails.data) {
      // If data is directly in the response
      setSelectedCrmLead({
        ...lead,
        branches: crmDetails.branches || [],
        total_branches: crmDetails.total_branches || 0,
        total_conversations: crmDetails.total_conversations || 0,
        all_conversations: crmDetails.all_conversations || []
      });
      setIsCrmViewOpen(true);
    } else {
        ...
        const updatedLeads = leads.map((l) => {
          if (l.id === selectedLead.id) {
            const newInteraction = {
              ...
              franchiseStatus: conversationData.franchiseStatus || "",
            };
            return {
              ...l,
  ...
  return (
    <div className="p-4 h-screen flex flex-col font-['Calibri'] bg-gray-50/50">
      ...
      <button
        onClick={() => handleAction1(lead.client_id || lead.id, "crmView")}
