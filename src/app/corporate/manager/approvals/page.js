...
export default function ManagerApprovals() {
  ...
  const fetchPendingExpenses = useCallback(async ({ isRefresh = false } = {}) => {
    ...
    try {
      const response = await API.apiGet("/api/corporate/manager/approvals/pending-expenses");
  ...
  const handleApprove = async (exp_id) => {
    try {
      const response = await API.apiPost("/api/corporate/manager/approvals/approve-expense", { exp_id });
      const data = await response.json();
      if (data.success) {
        fetchPendingExpenses();
      }
  ...
  const handleReject = async (exp_id) => {
    try {
      const response = await API.apiPost("/api/corporate/manager/approvals/reject-expense", { exp_id });
      const data = await response.json();
      if (data.success) {
        fetchPendingExpenses();
      }
  ...
  const handleSendToHR = async (exp_id) => {
    try {
      const response = await API.apiPost("/api/corporate/manager/approvals/send-to-hr", { exp_id });
      const data = await response.json();
      if (data.success) {
        fetchPendingExpenses();
      } else {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/corporate/manager/approvals/pending-expenses?${params.toString()}`, {
  ...
  {/* KPI DASHBOARD */}
      ...
      <div className="bg-white shadow-md rounded-xl border border-gray-100 overflow-hidden flex-1 flex flex-col min-h-[420px]">
        ...
        <div className="bg-gray-50 p-3 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 text-[#103c7f] shrink-0">
           ...
           <p className="text-[10px] font-black uppercase tracking-widest">Total Approval Value: <span className="text-lg italic">₹{(kpis.totalApprovedValue || 0).toLocaleString('en-IN')}</span></p>
        </div>
      </div>
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 bg-[#103c7f]/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-['Calibri'] animate-in fade-in duration-200">
          <div className="bg-white rounded-[24px] shadow-2xl max-w-4xl w-full p-8 relative overflow-hidden">
            ...
            <div className="flex justify-center">
              <Image src={previewExpense?.file_link} alt="Bill Preview" className="max-w-full max-h-[70vh] object-contain rounded-lg shadow-lg" />
            </div>
          </div>
        </div>
      )}
