...
export default function RCDomesticTargetPage() {
  ...
  const fetchMyTargets = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        // Fetch dynamic achievements for CV Parse KPIs
  ...
  // Fetch dynamic achievements for CV Parse and Tracker Sent KPIs
  const fetchDynamicAchievements = async (targets) => {
    try {
      ...
      // Get unique month-year combinations for CV Parse, Tracker Sent, Accuracy, Conversion, and Joining KPIs
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4 gap-4">
             <div>
                <h1 className="text-2xl font-black text-[#103c7f] uppercase tracking-tight flex items-center gap-2">
                    <CheckCircle size={24} className="text-emerald-500"/> Domestic Targets (RC)
                </h1>
