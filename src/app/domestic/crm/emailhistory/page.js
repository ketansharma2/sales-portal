...
export default function EmailHistoryPage() {
    ...
    // Send to Revenue Modal States (same as corporate)
    ...
    // Send to Revenue - Exact same flow as corporate
    const sendToRevenueTeam = (row) => {
        console.log("Preparing to send to revenue with data:", row);
        setRevenueForm({
            joiningDate: '',
            paymentFrom: '',
            tlName: row.tl_name || '',
            ...
            clientName: row.company_name || row.clientCompany || '',
    ...
    const handleSendToRevenue = async () => {
        ...
        if (!revenueForm.offerSalary || !revenueForm.terms || !revenueForm.paymentDays) {
            alert('Please fill required fields: Offer Salary, Terms %, Payment Days');
            return;
        }
        ...
        try {
            ...
            // You can call fetchEmailHistory() here if you have the function exposed

        } catch (err) {
            console.error(err);
            alert('Error sending to Revenue: ' + err.message);
        } finally {
                                                ...
                                                <div className="bg-gradient-to-br from-emerald-50 to-white rounded-xl p-4 border-2 border-emerald-100 relative">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                        ...
                                                        <div>
                                                         ...
                                                         {/* Retention Month - NEW INPUT */}
<div>
    <div className="flex items-center justify-between mb-1.5">
        <label className="text-[10px] font-black text-emerald-600 uppercase">Retention Month</label>
        {revenueForm.retentionMonth !== '' && revenueForm.retentionMonth !== null && revenueForm.retentionMonth !== undefined ? (
            <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded">
                {revenueForm.retentionMonth}
            </span>
        ) : null}
    </div>
    <div className="relative">
...
{/* Retention Amount - NEW INPUT */}
<div>
    <div className="flex items-center justify-between mb-1.5">
        <label className="text-[10px] font-black text-emerald-600 uppercase">Retention Amount (₹)</label>
        {revenueForm.retentionAmount !== '' && revenueForm.retentionAmount !== null && revenueForm.retentionAmount !== undefined ? (
            <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded">
                ₹ {parseFloat(revenueForm.retentionAmount).toLocaleString('en-IN')}
            </span>
        ) : null}
    </div>
    <div className="relative">
        ...
        <input 
            type="number" 
            className="w-full border border-emerald-200 rounded-lg p-2.5 pl-9 text-sm font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 bg-white/70"
           value={revenueForm.retentionAmount || ""}
            onChange={(e) => setRevenueForm({...revenueForm, retentionAmount: e.target.value})}
