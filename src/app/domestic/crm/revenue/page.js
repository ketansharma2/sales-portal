...
export default function RevenuePage() {
  ...
  const fetchRcUsers = async () => {
    try {
      ...
      const response = await fetch('/api/domestic/crm/rc-users', {
        headers: {
  ...
  const fetchCandidateFollowups = async (candidateId) => {
    try {
      ...
      const response = await fetch(`/api/domestic/crm/revenue/candidate-followup?candidate_id=${candidateId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSaveFollowup = async () => {
    try {
      ...
      const response = await fetch('/api/domestic/crm/revenue/payment-followup', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
