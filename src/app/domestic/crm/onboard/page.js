...
export default function OnboardPage() {
  ...
  // --- 2. MOVED DATA INTO STATE (Taaki click hone par update ho sake) ---
  const [onboardingList, setOnboardingList] = useState([
    { 
      ...
      isAcknowledged: false // Abhi False hai
    },
    { 
      id: 102, 
      ...
      isAcknowledged: true // Ye pehle se Done hai
    },
    { 
      id: 103, 
  ...
  // --- 3. NEW FUNCTION: Toggle Status Logic ---
  const handleAcknowledge = (id) => {
    // Ye function list ko update karega aur UI refresh karega
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between shrink-0 shadow-sm z-10">
          <div className="relative w-80">
            ...
            <input type="text" placeholder="Search by Company, Name or State..." className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold focus:outline-none focus:border-[#103c7f] text-gray-700 transition-all" />
          </div>
          <div className="flex gap-3">
             ...
             <button className="flex items-center gap-2 bg-[#103c7f] text-white px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-blue-900 transition-all shadow-md">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                  ...
                  <th className="px-5 py-4 whitespace-nowrap">Remarks by Sales Team</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {onboardingList.map((item) => (
                  ...
                  >
                    <td className="px-5 py-4 text-xs font-bold text-gray-500 whitespace-nowrap">{item.date}</td>
                    ...
                    <td className="px-5 py-4">
                      <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-[10px] font-bold border border-gray-200">{item.category}</span>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2 group/tooltip relative w-fit">
                        ...
                        <span className="text-xs font-medium text-gray-600 truncate max-w-[150px]">{item.remarks}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-center" onClick={(e) => e.stopPropagation()}>
                       {item.isAcknowledged ? (
                         ...
                         <button 
                           onClick={() => handleAcknowledge(item.id)} // <-- Ab ye function call hoga
                           className="bg-white border border-gray-300 text-gray-500 text-[10px] font-black px-3 py-1.5 rounded-lg hover:bg-[#103c7f] hover:text-white hover:border-[#103c7f] transition-all shadow-sm active:scale-95 uppercase tracking-wide"
                         >
                           Acknowledge
                         </button>
                       )}
