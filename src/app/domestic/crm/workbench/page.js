...
export default function CRMWorkbenchReport() {
    ...
    // Filter TLs: role contains TL and sector = Domestic
    const tlList = useMemo(() => {
        return allUsers
            .filter(u => u.role && u.role.includes('TL') && u.sector === 'Domestic')
            .map(u => ({ user_id: u.user_id, name: u.name }));
    }, [allUsers]);

    // Filter RCs: role contains RC and sector = Domestic
    const allRecruiters = useMemo(() => {
        return allUsers
            .filter(u => u.role && u.role.includes('RC') && u.sector === 'Domestic')
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-['Calibri'] p-2 md:p-2">
            ...
            {/* --- HEADER & FILTERS --- */}
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-2 gap-6 bg-white p-3 rounded-2xl shadow-sm border border-gray-200">
                <div>
