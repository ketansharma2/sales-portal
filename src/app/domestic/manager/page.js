      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 min-h-0 pb-2">
        ...
        <div className="flex-1 bg-white rounded-[24px] border border-gray-100 shadow-sm flex flex-col overflow-hidden">
           <div className="p-5 border-b border-gray-50 flex justify-between items-center shrink-0">
              ...
              <button onClick={() => { const today = new Date().toISOString().split('T')[0]; router.push(`/domestic/manager/team-leads?from_date=${today}&to_date=${today}`); }} className="text-[10px] font-black text-[#103c7f] uppercase bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-[#103c7f] hover:text-white transition-all">View Full Report</button>
           </div>
                    ...
                    {activityLoading ? (
                       ...
                       todayActivity.map((member) => (
                          <tr key={member.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors group">
                             ...
                             <td className="px-4 py-3 text-center"><button onClick={() => { const today = new Date().toISOString().split('T')[0]; router.push(`/domestic/manager/team-leads?from_date=${today}&to_date=${today}&selectedFse=${member.id}`); }} className="p-1.5 text-gray-400 hover:text-[#103c7f] hover:bg-blue-50 rounded-lg transition-all" title="View Detailed Log"><Eye size={16} /></button></td>
                          </tr>
                       ))
                    )}
                 </tbody>
