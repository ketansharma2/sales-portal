...
export default function HODPersonalClaims() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      // Call API to save
    const session = JSON.parse(localStorage.getItem('session') || '{}');
    const response = await fetch('/api/hod/expenses', {
      method: 'POST',
      headers: {
  ...
  const handleSubmitToAdmin = async (exp_id) => {
    try {
      const session = JSON.parse(localStorage.getItem('session') || '{}');
      const response = await fetch('/api/hod/submit-expense', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.access_token}`
                  ...
                  <td className="px-6 py-4 text-center">
                    {exp.status === "DRAFT" ? (
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleSubmitToAdmin(exp.id)} className="bg-[#103c7f] text-white px-4 py-2 rounded-[12px] hover:bg-[#a1db40] hover:text-[#103c7f] flex items-center gap-2 shadow-md transition-colors">
                          <Send size={12} /> <span className="text-[10px] font-black uppercase">Submit to Admin</span>
                        </button>
