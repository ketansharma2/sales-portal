...
function ClientFullViewModal({ lead, onClose, onEditInteraction }) {
    ...
    useEffect(() => {
        const fetchInteractions = async () => {
            if (!lead?.client_id) {
                // FALLBACK TO DUMMY DATA IF NO ID (For UI viewing purposes)
                setTimeout(() => {
                    setHistory([{
                        interaction_id: 1, contact_date: "07-Mar-2026", contact_mode: "Call", contact_person: "Priya Singh",
                        contact_no: "+91 91234 56789", email: "priya@skyline.com", remarks: "Client is interested, requested CTC breakdown.",
                        next_follow_up: "09-Mar-2026", status: "Interested", sub_status: "CTC Shared"
                    }]);
                    setLoading(false);
                }, 800);
                return;
            ...
            <div className="bg-[#f8fafc] rounded-[2rem] shadow-2xl w-full max-w-6xl max-h-[95dvh] overflow-hidden flex flex-col border border-white/50 animate-in zoom-in-95 duration-200">
                ...
                <div className="overflow-y-auto custom-scrollbar flex-1 p-6 md:p-8 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <StatCard label="Sourcing Date" value={lead?.sourcing_date || lead?.lastInteractionDate} icon={Calendar} />
                        <StatCard label="Category" value={lead?.category} icon={Zap} />
                        <StatCard label="Sourcing Mode" value={lead?.sourcing_mode || "Direct Calling"} icon={Phone} />
                        <StatCard label="Emp Count" value={lead?.empCount || lead?.emp_count} icon={User} />
                        <StatCard label="Reference" value={lead?.reference} icon={MessageSquarePlus} />
                        <StatCard label="Current Status" value={lead?.status} icon={CheckCircle} colorClass="bg-blue-50 text-blue-600"/>
                        <StatCard label="Projection" value={lead?.projection} icon={Briefcase} colorClass="bg-emerald-50 text-emerald-600 "/>
                    </div>
    ...
    const detailedData = [
        {
            ...
            projection: "WP<50", // Projection instead of Franchise
            owner: "Aman Gupta",
        ...
        {
            id: 2,
            companyName: "FreshBites Retail",
            contactName: "Rohit Mehta",
            contactNumber: "+91 99887 76655",
            lastInteraction: "Shared company profile and CTC details",
            lastInteractionDate: "Mar 8, 2026",
            nextFollowup: "Mar 11, 2026",
            status: "Interested",
            substatus: "CTC Shared",
            projection: "WP>50", // Projection instead of Franchise
            owner: "Riya Desai",
            category: "FMCG",
            city: "Delhi",
                            ...
                            {detailedData.map((row) => (
                                <tr key={row.id} className="hover:bg-orange-50/30 transition-colors group">
                                    ...
                                    <td className="py-3 px-4">
                                        ...
                                        <p className="text-[10px] font-black text-orange-600 mt-0.5">{row.lastInteractionDate}</p>
                                    </td>
