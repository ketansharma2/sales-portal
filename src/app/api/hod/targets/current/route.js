...
// GET - Fetch current month's targets for HOD with achievements
export async function GET(request) {
  try {
    ...
    // Get current month (YYYY-MM-DD format)
    ...
    // Fetch current month's targets for these managers
    ...
    if (targetsError) {
      console.error('Current targets fetch error:', targetsError)
      return NextResponse.json({
    ...
    // Get all FSEs and Leadgens under these managers
    ...
    const targetsWithAchievements = await Promise.all((targets || []).map(async (target) => {
      ...
      const targetMonth = target.month ? target.month.substring(0, 7) : null
