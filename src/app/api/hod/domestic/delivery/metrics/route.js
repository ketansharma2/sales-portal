...
if (authError || !user) {
  ...
  return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    ...
    // === CRM METRICS (using domestic_crm_interview) ===
    let crmWbQuery = supabaseServer.from('domestic_workbench').select('req_id, sent_to_rc, date');
    ...
    const { data: crmInterviews } = crmReqIds.length > 0 
      ? await supabaseServer.from('domestic_crm_interview').select('interview_status').in('req_id', crmReqIds)
      : { data: [] };
    ...
    // === TL METRICS ===
    let tlWbQuery = supabaseServer.from('domestic_workbench').select('req_id, sent_to_tl, date');
    ...
    const { data: tlInterviews } = tlReqIds.length > 0 
      ? await supabaseServer.from('domestic_crm_interview').select('interview_status').in('req_id', tlReqIds)
      : { data: [] };
    ...
    // === RC METRICS ===
    let rcWbQuery = supabaseServer.from('domestic_workbench').select('req_id, sent_to_rc, date');
