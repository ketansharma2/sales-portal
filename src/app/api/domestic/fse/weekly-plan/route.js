...
const { data: assignments, error: assignError } = await supabaseServer
  ...
  .order('date', { ascending: true });
    ...
    const leadIds = assignments?.map(a => a.domestic_leadgen_leads?.client_id).filter(id => id) || [];
    let latestRemarks = {};
    if (leadIds.length > 0) {
    ...
    const formattedAssignments = assignments?.map(assignment => ({
      id: assignment.id,
      client_id: assignment.domestic_leadgen_leads?.client_id,
      company: assignment.domestic_leadgen_leads?.company,
      location: assignment.domestic_leadgen_leads?.location,
      state: assignment.domestic_leadgen_leads?.state,
      emp_count: assignment.domestic_leadgen_leads?.emp_count,
      category: assignment.domestic_leadgen_leads?.category,
      sourcing_date: assignment.domestic_leadgen_leads?.sourcing_date,
      reference: assignment.domestic_leadgen_leads?.reference,
      visit_date: assignment.date,
      sm_status: assignment.sm_status,
      fse_status: assignment.fse_status,
      created_at: assignment.created_at,
      latestRemark: latestRemarks[assignment.domestic_leadgen_leads?.client_id] || "Assigned for visit"
    })) || []
