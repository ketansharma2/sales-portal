...
export async function GET(request) {
  try {
    ...
    if (fetchError) {
      console.error('Count rejected by client error:', fetchError)
      return NextResponse.json({
        error: 'Failed to count rejected by client',
    ...
    return NextResponse.json({
      success: true,
      data: {
        rejectedByClient: count || 0
      }
    })

  } catch (error) {
    console.error('CRM rejected by client API error:', error)
    return NextResponse.json({
