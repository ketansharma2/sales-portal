...
export async function GET(request) {
  try {
    ...
    if (tracksError) {
      console.error('Fetch domestic candidate track error:', tracksError)
    ...
    console.error('Domestic candidate-track GET error:', error)
...
export async function POST(request) {
  try {
    ...
    const { revenue_id, date, next_follow_up, candidate_status, remarks } = body
    ...
    if (insertError) {
      console.error('Insert domestic candidate track error:', insertError)
      return NextResponse.json({ error: 'Failed to save candidate tracking log' }, { status: 500 })
    }

    return NextResponse.json({ success: true, data: { ...track, ...insertData } }, { status: 201 })

  } catch (error) {
    console.error('Domestic candidate-track POST error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
