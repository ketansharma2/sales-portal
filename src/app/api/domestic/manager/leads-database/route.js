...
export async function GET(request) {
  try {
    ...
    // Fetch leads from domestic_leadgen_leads for all leadgens under this manager
    // With LEFT JOIN to domestic_leads_interaction to get latest interaction
    ...
    const formattedLeads = rawData?.map((lead) => {
      const latestInteraction = lead.domestic_leads_interaction?.[0] || null
