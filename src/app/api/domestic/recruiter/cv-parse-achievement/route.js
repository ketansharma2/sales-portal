...
if (authError || !user) {
  ...
  return NextResponse.json({ success: false, error: 'Unauthorized' }, { status: 401 });
    ...
    console.log('CV Parse achievement - Month:', month, 'Year:', year, 'User:', user.id);

    // Count CVs parsed in the month (filter by user_id and portal_date)
    ...
    console.log('CV parse data:', cvParseData?.length || 0);
    ...
    console.log('CV parse achieved:', achieved);
    ...
    console.error('CV Parse achievement API error:', error);
    return NextResponse.json({
