...
export async function GET(request) {
  try {
    ...
    // Count all interactions where status is NOT "Not Picked"
    const pickedInteractions = (interactionsData || []).filter(
      i => i.status?.toLowerCase() !== 'not picked'
    );

    const totalPicked = pickedInteractions.length;

    /* ---------------- STARTUP COUNT ---------------- */
    const startupPicked = pickedInteractions.filter(i => {
