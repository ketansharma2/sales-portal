...
export async function POST(request) {
  try {
    ...
    const { data: track, error: insertError } = await supabaseServer
     ...
     if (insertError) {
       return NextResponse.json({ error: 'Failed to save client log' }, { status: 500 })
     }

     // Get user name for loggedBy
 ...
 // GET: Fetch client track records for a revenue
...
if (authError || !user) {
  ...
  return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
     ...
     if (fetchError) {
       return NextResponse.json({ error: 'Failed to fetch client track data' }, { status: 500 })
     }
