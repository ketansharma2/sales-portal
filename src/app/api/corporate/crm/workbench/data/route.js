...
export async function GET(request) {
  try {
    ...
    // Apply TL filter (sent_to_tl)
    if (tlId) {
      query = query.eq('sent_to_tl', tlId)
    }

    // Apply RC filter (sent_to_rc)
    if (rcId) {
    ...
    const countsPromises = workbenchData.map(async (item) => {
      ...
      // Get conversation_ids for this req/RC/date
      ...
      // Tracker sent count (sent_to_crm is not null and crm_sent_date = item.date)
