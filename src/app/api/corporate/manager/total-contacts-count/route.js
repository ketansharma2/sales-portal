...
export async function GET(request) {
  try {
    ...
    if (leadgenIds.length === 0) {
      return NextResponse.json({ success: true, data: { contacts: { total: 0 } } });
    }
    ...
    const totalContacts = latestInteractionPerContact.size;
    ...
    console.error('Total contacts count API error:', error);
