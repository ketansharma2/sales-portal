...
export async function GET(request) {
  try {
    ...
    if (leadgenIds.length === 0) {
      return NextResponse.json({ success: true, data: { onboard: { total: 0 } } });
    }
    ...
    const { data: distinctClientsData, error: distinctError } = await query.select('client_id');
    ...
    const totalOnboard = uniqueClientIds.length;
    ...
    console.error('Onboard count API error:', error);
