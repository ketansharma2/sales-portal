...
export async function GET(request) {
  try {
    ...
    const franchiseStatus = searchParams.get('status') || '';
    ...
    if (leadgenIds.length === 0) {
      return NextResponse.json({ success: true, data: { franchise: { total: 0 } } });
    }
    ...
    const totalFranchise = filteredFirstFranchise.length;
    ...
    console.error('Franchise form count API error:', error);
