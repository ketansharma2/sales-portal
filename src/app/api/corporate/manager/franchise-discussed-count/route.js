...
export async function GET(request) {
  try {
    ...
    if (leadgenIds.length === 0) {
      return NextResponse.json({ success: true, data: { franchise: { discussed: { total: 0 } } } });
    }
    ...
    const totalDiscussed = filteredFirstFranchise.length;

    return NextResponse.json({
      success: true,
      data: {
        franchise: { discussed: { total: totalDiscussed } }
      }
    });

  } catch (error) {
    console.error('Franchise Discussed count API error:', error);
