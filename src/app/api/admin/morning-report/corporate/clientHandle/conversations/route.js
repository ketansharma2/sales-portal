...
export async function GET(request) {
  try {
    ...
    // Build query for conversations with user_id
    let query = supabaseServer
      .from('corporate_crm_conversation')
      .select('conversation_id, branch_id, date, mode, contact_name, discussion, user_id')
    ...
    const userIds = [...new Set(conversations.map(c => c.user_id).filter(id => id))]
    ...
    if (userIds.length > 0) {
      ...
      if (usersError) {
        ...
        userMap = users.reduce((map, user) => {
          map[user.user_id] = user.name || user.email || 'Unknown User'
          return map
        }, {})
      }
    }
    ...
    const conversationData = conversations.map(conversation => {
      ...
      return {
        ...
        user_name: userMap[conversation.user_id] || 'Unknown User',
