...
export async function GET(request) {
  try {
    ...
    // Build query for domestic workbench data with date range filter
    ...
    if (fetchError) {
      console.error('Fetch domestic workbench error:', fetchError)
      return NextResponse.json({
    ...
    // Fetch advance_sti counts from domestic_workbench_sti
    ...
    // Fetch tracker_sent_by_rc counts from candidates_conversation (sent_to_tl is not null and sent_date = date)
    ...
    // Fetch tracker_sent_by_tl counts from candidates_conversation (sent_to_crm is not null and crm_sent_date in range)
    ...
    // Fetch CRM users (creator of workbench entry)
    ...
    console.error('Domestic Workbench API error:', error)
    return NextResponse.json({
