      ...
      >
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-bold text-[#103c7f]">
...
export default function HRReimbursementPage() {
  ...
  const [selectedClaims, setSelectedClaims] = useState(new Set()); // Track selected claims for bulk approve
  const [hrVerifiedClaims, setHrVerifiedClaims] = useState(new Set()); // Track HR verified claims
  // --- FETCH DATA FROM API ---
  useEffect(() => {
    fetchPendingPayouts();
    fetchPaymentHistory();
  ...
  const totalPayable = pendingClaims.reduce(
  ...
  // Handle individual claim selection for bulk approve
  ...
  const handleSelectAll = () => {
    if (selectedClaims.size === filteredPending.length) {
      setSelectedClaims(new Set());
    } else {
      const allIds = filteredPending.map((claim) => claim.id);
      setSelectedClaims(new Set(allIds));
    }
  };

  // Handle HR verification check (individual)
  const handleHrVerify = (claimId) => {
    setHrVerifiedClaims((prev) => {
      const newSet = new Set(prev);
      if (newSet.has(claimId)) {
        newSet.delete(claimId);
      } else {
      ...
      <div className="bg-[#103c7f] h-16 px-6 border-b border-[#0d316a] shadow-md flex justify-between items-center shrink-0 z-20">
        ...
        <div className="flex items-center gap-3 bg-blue-900/30 px-4 py-2 rounded-lg border border-blue-400/20">
          <div className="text-right">
            <p className="text-[9px] text-blue-200 uppercase font-bold">
              Total Payable
            </p>
      ...
      <div className="px-6 py-4 bg-white border-b border-gray-200 shadow-sm flex justify-between items-center shrink-0">
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          ...
          >
            ...
            <span className="bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full text-[9px]">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-colors ${queueFilter === "all" ? "bg-[#103c7f] text-white border-[#103c7f]" : "bg-white text-gray-500 border-gray-200"}`}
              >
                All
              </button>
              <button
                onClick={() => setQueueFilter("fse")}
                className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-colors ${queueFilter === "fse" ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-white text-gray-500 border-gray-200"}`}
              >
                FSE
              </button>
              <button
                onClick={() => setQueueFilter("manager")}
                className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-colors ${queueFilter === "manager" ? "bg-orange-100 text-orange-700 border-orange-200" : "bg-white text-gray-500 border-gray-200"}`}
              >
                Managers
              </button>
              <button
                onClick={() => setQueueFilter("hod")}
                className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-colors ${queueFilter === "hod" ? "bg-blue-100 text-blue-700 border-blue-200" : "bg-white text-gray-500 border-gray-200"}`}
              >
                HODs
              </button>
            </div>
        ...
        {mainTab === "history" && (
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold w-64 outline-none focus:border-[#103c7f]"
            />
          </div>
        )}
      </div>
                              ...
                              {claim.file_link && (
                                <button
                                  onClick={() =>
                                    handlePreview(
                                      claim.file_link,
                                      `${claim.empName}_${claim.category}_Receipt`,
                                    )
                                  }
                        ...
                        >
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-sm font-bold">
                              No pending payouts in this queue.
                            </p>
                          </div>
                        </td>
                      </tr>
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col gap-1 items-start">
                              ...
                              <p
                                className="text-[11px] text-gray-600 italic truncate max-w-[180px]"
                                title={item.desc}
                              >
                                "{item.desc}"
                              </p>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-start gap-2">
                              <span className="font-bold text-gray-800 text-sm w-20">
                                ₹ {item.amount}
                              </span>
                              {item.file_link && (
                                <button
                                  onClick={() =>
                                    handlePreview(
                                      item.file_link,
                                      `${item.empName}_${item.category}_Receipt`,
                                    )
                                  }
