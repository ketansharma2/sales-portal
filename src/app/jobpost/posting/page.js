...
export default function JobPosterPanel() {
  ...
  const fetchJDs = async () => {
    try {
      setError(null);
      const response = await API.apiGet('/api/jobpost/posting');
...
// Handle Status Change from dropdown
const handleStatusChange = async (jdId, newStatus, jobType) => {
  // Find the posting to get its sector/job_type
  const post = postings.find(p => p.id === jdId);
  if (!post) return;
  
  try {
    const session = JSON.parse(localStorage.getItem('session') || '{}');
    
    // ✅ Call the new unified status API
    const response = await  API.apiPut('/api/jobpost/posting', {
        jd_id: jdId, 
        status: newStatus,
        job_type: jobType || post.job_type // Pass job_type from the post
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-lg uppercase tracking-wide">Document Preview</h3>
                             <p className="text-[10px] text-blue-200 font-bold tracking-widest uppercase">For Client: {selectedJD.req_data?.client_name || selectedJD.client_name}</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white w-full max-w-[210mm] min-h-[297mm] h-max mx-auto p-[10mm] md:p-[15mm] shadow-xl text-black font-['Calibri'] relative print:shadow-none print:m-0" id="pdf-content">
                        ...
                        <div className="border border-black p-8 min-h-[850px] relative print:border-none print:p-0">
                             ...
                             <div className="space-y-4 mb-10 text-[15px] leading-relaxed">
                                 {(selectedJD.req_data?.job_title || selectedJD.job_title) && <p><span className="font-bold">JOB TITLE : </span> {selectedJD.req_data?.job_title || selectedJD.job_title}</p>}
                                 {(selectedJD.req_data?.location || selectedJD.location) && <p><span className="font-bold">LOCATION : </span> {selectedJD.req_data?.location || selectedJD.location}</p>}
                                 {selectedJD.req_data?.experience && <p><span className="font-bold">EXPERIENCE : </span> {selectedJD.req_data.experience}</p>}
                                 {selectedJD.req_data?.employment_type && <p><span className="font-bold">EMPLOYMENT TYPE : </span> {selectedJD.req_data.employment_type}</p>}
                                 {selectedJD.req_data?.working_days && <p><span className="font-bold">WORKING DAYS : </span> {selectedJD.req_data.working_days}</p>}
                                 {selectedJD.req_data?.timings && <p><span className="font-bold">TIMINGS : </span> {selectedJD.req_data.timings}</p>}
                                 {(selectedJD.req_data?.package || selectedJD.package) && <p><span className="font-bold">PACKAGE : </span> {selectedJD.req_data?.package || selectedJD.package}</p>}
                                 {selectedJD.req_data?.tool_req && <p><span className="font-bold">TOOL REQUIREMENT : </span> {selectedJD.req_data.tool_req}</p>}
                             </div>

                             <div className="space-y-8 text-[15px]">
                                 {selectedJD.req_data?.job_summary && <div><h4 className="font-bold mb-2 uppercase text-[16px]">Job Summary :</h4><p className="leading-relaxed text-justify text-gray-800">{selectedJD.req_data.job_summary}</p></div>}
                                 {selectedJD.req_data?.rnr && <div><h4 className="font-bold mb-2 uppercase text-[16px]">Role & Responsibilities :</h4><ul className="list-disc pl-5 space-y-1.5 text-gray-800">{selectedJD.req_data.rnr?.split('\n').map((line, i) => line.trim() && <li key={i}>{line}</li>)}</ul></div>}
                                 {selectedJD.req_data?.req_skills && <div><h4 className="font-bold mb-2 uppercase text-[16px]">Required Skills :</h4><ul className="list-disc pl-5 space-y-1.5 text-gray-800">{selectedJD.req_data.req_skills?.split('\n').map((line, i) => line.trim() && <li key={i}>{line}</li>)}</ul></div>}
                                 {selectedJD.req_data?.preferred_qual && <div><h4 className="font-bold mb-2 uppercase text-[16px]">Preferred Qualifications :</h4><ul className="list-disc pl-5 space-y-1.5 text-gray-800">{selectedJD.req_data.preferred_qual?.split('\n').map((line, i) => line.trim() && <li key={i}>{line}</li>)}</ul></div>}
                                 {selectedJD.req_data?.company_offers && <div><h4 className="font-bold mb-2 uppercase text-[16px]">What Company Offer :</h4><ul className="list-disc pl-5 space-y-1.5 text-gray-800">{selectedJD.req_data.company_offers?.split('\n').map((line, i) => line.trim() && <li key={i}>{line}</li>)}</ul></div>}
                                 {selectedJD.req_data?.contact_details && <div className="mt-12 pt-6 border-t border-black/20"><h4 className="font-bold mb-3 uppercase text-[16px]">Contact Us To Apply :</h4><div className="whitespace-pre-line leading-loose text-gray-900 font-medium">{selectedJD.req_data.contact_details}</div></div>}
                             </div>
